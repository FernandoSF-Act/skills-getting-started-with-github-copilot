...
activities = {
    ...
    "Drama Club": {
        "description": "Performance art, theater productions, and acting workshops",
...
def signup_for_activity(activity_name: str, email: str):
    ...
    # Validar que o estudante ainda não está inscrito
    if email in activity["participants"]:
        raise HTTPException(status_code=400, detail="Student already signed up for this activity")

    # Check if activity is at max capacity
    if len(activity["participants"]) >= activity["max_participants"]:
        raise HTTPException(status_code=400, detail="This activity has no spots left")
